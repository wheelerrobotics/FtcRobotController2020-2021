...
public class NoTurnAround extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        TrajectorySequence park = drive.trajectorySequenceBuilder(new Pose2d(-32, -5, PI/6))
                .addTemporalMarker(0.2, () -> { // maybe make spatial in future
                    bot.setClawTarget(Heights.clawClosed);
                    bot.setWristTarget(Heights.levelWristPickup);
                    bot.setArmTarget(Heights.levelArmPickup);
                ...
                .lineTo(new Vector2d((conePos == 1 ? 61.5 : (conePos == 2 ? 35 : -34)), -34))
        ...
        while (opModeIsActive()) {
            ...
            if (currentMovementID == 1 ||
                    ...
                    currentMovementID == 5 ||
                    currentMovementID == 7 ||
                    currentMovementID == 9) drive.followTrajectorySequenceAsync(cycleSplineToStack);
            if (currentMovementID == 2 ||
                    currentMovementID == 4 ||
                    currentMovementID == 6 ||
                    currentMovementID == 8 ||
                    currentMovementID == 10) drive.followTrajectorySequenceAsync(cycleSplineToJunction);
            if (currentMovementID == 11) drive.followTrajectorySequenceAsync(park);
        }
    }
